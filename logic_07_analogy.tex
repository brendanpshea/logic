...
\begin{frame}{Scientific Analogy: Rutherford's Solar System Model of the Atom}
	...
	\begin{center}
		\begin{tikzpicture}
			% Solar System (Source)
			\node at (-3,2) {Solar System};
			\draw[fill=yellow] (-3,0) circle (0.5);
			\draw[blue] (-3,0) circle (1.5);
			\draw[fill=blue!50] (-1.8,0.8) circle (0.1);
			
			% Atom (Target)
			\node at (3,2) {Atomic Model};
			\draw[fill=red] (3,0) circle (0.5);
			\draw[blue] (3,0) circle (1.5);
			\filldraw[black] (4.2,0.8) circle (0.1);
			
			% Arrows indicating analogical mapping
			\draw[->, thick] (-2.5,0) -- (2.5,0) node[midway, above] {Analogy};
			\draw[->, thick] (-1.8,0.8) -- (4.2,0.8) node[midway, above] {Analogy};
			
			% Labels
			\node[below] at (-3,-0.7) {Sun (massive center)};
			\node[below] at (3,-0.7) {Nucleus (positive charge)};
			\node[right] at (-1.7,0.8) {Planet};
			\node[right] at (4.3,0.8) {Electron};
		\end{tikzpicture}
	\end{center}
\end{frame}
